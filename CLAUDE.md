...
### 작업 파일 및 태스크 관리 (시스템 규칙)
- **일일 작업 로그**: 작업(코딩/테스트 등)을 완료할 때마다 반드시 `docs/tasks/daily_log.md`를 업데이트하여 상태(In Progress -> Done 등)를 최신화할 것.
- **설계 문서 연동**: 설계 에이전트(Gemini)가 `docs/tasks/`에 생성한 `[날짜]_[기능]_plan.md` 및 `_task.md`를 최우선으로 읽고 지시사항을 따를 것.
