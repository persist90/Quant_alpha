---
description: 새 팩터를 시스템에 추가하고 IC 검증까지 자동 수행
...
- 단순 구조: Step 2A로, 복잡 구조(DART 재무제표 등): Step 2B로, 이미 있으면 Step 3으로

### Step 2A: 단순 데이터 커넥터 추가 (data-connector)
"data-connector 에이전트를 사용해서 [필요한 데이터]를 수집하는
커넥터를 src/ingestion/에 신규 생성. DB에 재고 데이터 저장."

### Step 2B: 복잡 데이터 파서 추가 (complex-parser)
"complex-parser 에이전트를 사용해서 [DART 재무제표 등]을 파싱하는
모듈을 src/ingestion/parsers/에 신규 생성."

### Step 3: 팩터 계산 구현 (factor-calculator)
...
| Step | 에이전트 | 실행 방식 | 이유 |
|------|---------|----------|------|
| Step 2A/2B | data-connector 또는 complex-parser | 순차 | 데이터 수집이 모든 후속 단계의 전제 |
| Step 3 | factor-calculator | 순차 | 커넥터/파서 완료 후에만 계산 가능 |
